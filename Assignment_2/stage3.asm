...
EnterMS:
      ...
      MOV R4, R0        ;Moving InputMS from R0 into R4 to save as replay purpose
Announcement:
      BL GoDownLine
...
Draw: 
      ...
      STR R2, .WriteString
PlayAgain:
      MOV R2, #PlayAgainPrompt ;"Play again (y/n) ?"
      STR R2, .WriteString
      MOV R1, R4        ;Loading saved InputMS
      MOV R0, #PlayAgainInput ;Reading Input
      STR R0, .ReadString
      LDRB R0, PlayAgainInput
      CMP R0, #121      ;Validating ReplayInput with "y"
      BEQ PlayerTurn
      CMP R0, #110      ;Validating ReplayInput with "n"
      BEQ GameOver
      CMP R0, #89       ;Validating ReplayInput with "Y"
      BEQ PlayerTurn
      CMP R0, #78       ;Validating ReplayInput with "N"
      BEQ GameOver
      B PlayAgain
GameOver:
      BL GoDownLine
