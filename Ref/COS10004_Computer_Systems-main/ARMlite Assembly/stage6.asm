...
lose:
      ...
      B stop
//
// Stage 6: Visualising guess history
//
draw:
      PUSH {R4, R5, R6, R7, R8, R9, R10}
      MOV R4, #0        // Index variable for loop 6, as well as for pixel drawing in line
      MOV R5, #querycode
      MOV R7, #0        // Variable storing color to draw
      MOV R8, #.PixelScreen
      MOV R9, R12       // Argument of this function: The index of current guess
// Make R9 be able to decide which lines will be drawed
      SUB R9, R9, #1
      LSL R9, R9, #8 
loop6:
      LDR R6, [R5]      // Store each 'color' char
      CMP R6, #114      // Compare the char with "r"
      BEQ cred
      CMP R6, #103      // Compare the char with "g"
      BEQ cgreen
      ...
      CMP R6, #99       // Compare the char with "c"
      BEQ ccyan
next6:                  // Continue the loop 6
      ADD R10, R9, R4   // Index for directly drawing pixel 
      STR R7, [R8 + R10]
      ADD R4, R4, #4 
      ADD R5, R5, #4
      CMP R4, #16       // Draw 4 pixels in this current line or not?
      BLT loop6
